...
function recipientPage(token: string, view: RecipientAuthorizationView, message?: string): string {
  ...
  if (view.state === 'claimed' && (view.phoneNumber || view.smsDelivered)) {
    ...
    if (view.smsDelivered) {
      ...
      // 仅等待短信、尚未宣告等待：显示点击获取验证码按钮，等待动画不显示。
      verificationMarkup = `<div class="verification-request-pending"><form method="post" action="/a/${encodeURIComponent(token)}/verification-request"><button type="submit">点击获取验证码</button></form></div>`;
    }
